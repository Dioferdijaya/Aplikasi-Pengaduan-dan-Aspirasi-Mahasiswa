    ...
    <div class="container">
        <div class="header">
            <div class="welcome">
                ...
                <img src="https://ui-avatars.com/api/?name=<?php echo urlencode($initials); ?>&background=0D8ABC&color=fff&size=100"
      ...
      <div class="profile-text">
        ...
        @if (isset($latestKritikSaran))
            ...
            <div class="message-box">
                ...
                <p><strong>Pesan:</strong> {{ $latestKritikSaran->isi_pesan ?? 'Pesan tidak tersedia.' }}</p>
                <p class="date">
                    {{ $latestKritikSaran->created_at ? $latestKritikSaran->created_at->format('d F Y H:i') : 'Tanggal tidak tersedia' }}
                </p>
            </div>
            ...
            @if (isset($latestKritikSaran->tanggapan))
                ...
                <div class="reply-box">
                    <p><strong>From:</strong> Admin
                        {{ $latestKritikSaran->tanggapan->admin->nama_lengkap ?? 'Fakultas MIPA' }}</p>
                    <p><strong>To:</strong> {{ $user->nama_lengkap ?? ($user->username ?? 'Anonymous') }}</p>
                    <p><strong>Pesan:</strong>
                        {{ $latestKritikSaran->tanggapan->isi_tanggapan ?? 'Isi tanggapan tidak tersedia.' }}</p>
                    <p class="date">
                        {{ $latestKritikSaran->tanggapan->created_at ? $latestKritikSaran->tanggapan->created_at->format('d F Y H:i') : 'Tanggal tidak tersedia' }}
                    </p>
                </div>
